...
async def grab_banners_for_ports(target: str, ports: List[int], timeout: int = 2.5, sem: int = 200):
    semaphore = asyncio.Semaphore(sem)
    ...
    tasks = [grab(port) for port in ports]
...
async def scan_host_tcp(target: str, ports: List[PortRange],
                        timeout: float = 2.5, sem: int = 500) -> dict:
    scan = await scan_ports(target, ports, timeout=timeout, sem=sem)
    ports = scan["open_ports"]
    results = await grab_banners_for_ports(target, ports, timeout=timeout, sem=min(sem, 200))
